...
def py_func1(array, top_boundary, obst_boundary):

	global indices, sdfunct, vert_OFtoNP, weights_OFtoNP, vert_NPtoOF, weights_NPtoOF, grid_shape_y, grid_shape_x, points, x_min, x_max, y_min, y_max, X0,Y0

	delta = 5e-3 

	x_min = round(np.min(array[...,2]),2)
	x_max = round(np.max(array[...,2]),2)

	y_min = round(np.min(array[...,3]),2)
	y_max = round(np.max(array[...,3]),2)
	...
	domain_bool, sdf = domain_dist(top_boundary, obst_boundary, xy0)
...
def py_func(array):
	...
	#global model, pcainput, pcap, sdfunct, grid_shape_y, grid_shape_x
	#global indices, vert_OFtoNP, weights_OFtoNP, vert_NPtoOF, weights_NPtoOF, grid_shape_y, grid_shape_x
	...
	t0 = time.time()
	Ux_interp = interpolate_fill(Ux_adim, vert_OFtoNP, weights_OFtoNP)
	Uy_interp = interpolate_fill(Uy_adim, vert_OFtoNP, weights_OFtoNP)
	t1 = time.time()
	...
	comp_p = pcap.components_
	pca_mean_p = pcap.mean_

	comp_input = pcainput.components_
	...
	input_transformed = np.dot(input_flat - pca_mean_input, comp_input.T[:,:PC_input])
	...
	t0 = time.time() #start timing
	res_flat_inv = np.dot(res_concat*max_abs_p_PCA, comp_p[:PC_p, :]) + pca_mean_p	
	...
	p_interp[np.isnan(p_interp)] = p_prev[np.isnan(p_interp)]
...
plt.scatter(array[:,2],array[:,3], c = array[:,4])
plt.show()

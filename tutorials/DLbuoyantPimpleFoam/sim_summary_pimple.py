...
def parse_logs(log_content):
    ...
    time_block_pattern = re.compile(
        r'Time = (?P<time>\d+\.\d+)\n'
        r'.*?diagonal:  Solving for rho, Initial residual = .*?, Final residual = .*?, No Iterations (?P<rho_iterations>\d+)\n'
        ...
        r'DILUPBiCGStab:  Solving for h, Initial residual = (?P<h_initial_residual>[\d.]+(?:e[+-]?\d+)?), Final residual = .*?, No Iterations (?P<h_iterations>\d+)\n'
        r'GAMG:  Solving for p_rgh, Initial residual = (?P<p_iter1_1_initial_residual>[\d.]+(?:e[+-]?\d+)?), Final residual = .*?, No Iterations (?P<p_iter1_1_iterations>\d+)\n'
        r'GAMG:  Solving for p_rgh, Initial residual = (?P<p_iter1_2_initial_residual>[\d.]+(?:e[+-]?\d+)?), Final residual = .*?, No Iterations (?P<p_iter1_2_iterations>\d+)\n'
        r'diagonal:  Solving for rho, Initial residual = .*?, Final residual = .*?, No Iterations (?P<rho2_iterations>\d+)\n'
        r'time step continuity errors : sum local = [\d.]+(?:e[+-]?\d+)?, global = [+-]?[\d.]+(?:e[+-]?\d+)?, cumulative = [+-]?[\d.]+(?:e[+-]?\d+)?\n'
        r'GAMG:  Solving for p_rgh, Initial residual = (?P<p_iter2_1_initial_residual>[\d.]+(?:e[+-]?\d+)?), Final residual = .*?, No Iterations (?P<p_iter2_1_iterations>\d+)\n'
        r'GAMG:  Solving for p_rgh, Initial residual = (?P<p_iter2_2_initial_residual>[\d.]+(?:e[+-]?\d+)?), Final residual = .*?, No Iterations (?P<p_iter2_2_iterations>\d+)\n'
        r'diagonal:  Solving for rho, Initial residual = .*?, Final residual = .*?, No Iterations (?P<rho3_iterations>\d+)\n'
        r'time step continuity errors : sum local = [\d.]+(?:e[+-]?\d+)?, global = [+-]?[\d.]+(?:e[+-]?\d+)?, cumulative = [+-]?[\d.]+(?:e[+-]?\d+)?\n'
...
def calculate_averages(matches):
    ...
    execution_times = [float(m[24]) for m in matches]
...
output_file = f'buoyantPimpleFoam_sim{folder_name}_summary.csv'

...
def parse_logs(log_content):
    # Regular expression to find each Time block and capture the necessary data for DLpisoFoam
    time_block_pattern = re.compile(
        r'Time = (?P<time>\d+\.\d+)\n+'
        r'.*?'
        r'deltat = \d+\.?\d*\n+'
        r'.*?'
        r'Courant Number mean: .*?\n'
        ...
        r'GAMG:\s+Solving for p, Initial residual = (?P<p_iter1_1_initial_residual>\d+\.?\d*e?-?\d*), Final residual = .*?, No Iterations (?P<p_iter1_1_iterations>\d+)\n'
        r'GAMG:\s+Solving for p, Initial residual = (?P<p_iter1_2_initial_residual>\d+\.?\d*e?-?\d*), Final residual = .*?, No Iterations (?P<p_iter1_2_iterations>\d+)\n'
        r'time step continuity errors : sum local = .*?, global = .*?, cumulative = .*?\n'
        r'GAMG:\s+Solving for p, Initial residual = (?P<p_iter2_1_initial_residual>\d+\.?\d*e?-?\d*), Final residual = .*?, No Iterations (?P<p_iter2_1_iterations>\d+)\n'
        r'GAMG:\s+Solving for p, Initial residual = (?P<p_iter2_2_initial_residual>\d+\.?\d*e?-?\d*), Final residual = .*?, No Iterations (?P<p_iter2_2_iterations>\d+)\n'
        r'time step continuity errors : sum local = .*?, global = .*?, cumulative = .*?\n'
...
def calculate_averages(matches):
    ...
    # Extract numeric data (adjusted indices for DLpisoFoam)
    ux_iters = [int(m[2]) for m in matches]
    uy_iters = [int(m[4]) for m in matches]
    uz_iters = [int(m[6]) for m in matches]
    dl_times = [float(m[7]) for m in matches]
    p_iter1_1_iters = [int(m[9]) for m in matches]
    p_iter1_2_iters = [int(m[11]) for m in matches]
    p_iter2_1_iters = [int(m[13]) for m in matches]
    p_iter2_2_iters = [int(m[15]) for m in matches]
    omega_iters = [int(m[17]) for m in matches]
    k_iters = [int(m[19]) for m in matches]
    execution_times = [float(m[20]) for m in matches]
...
output_file = f'DLpisoFoam_sim{folder_name}_summary.csv'

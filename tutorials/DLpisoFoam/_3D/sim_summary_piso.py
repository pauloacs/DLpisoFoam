...
def parse_logs(log_content):
    # Regular expression to find each Time block and capture the necessary data for pisoFoam
    time_block_pattern = re.compile(
        r'Time = (?P<time>\d+\.\d+)\n'
        r'.*?Courant Number mean: [\d.]+(?:e[+-]?\d+)? max: [\d.]+(?:e[+-]?\d+)?\n'
        r'smoothSolver:  Solving for Ux, Initial residual = (?P<ux_initial_residual>[\d.]+(?:e[+-]?\d+)?), Final residual = .*?, No Iterations (?P<ux_iterations>\d+)\n'
        r'smoothSolver:  Solving for Uy, Initial residual = (?P<uy_initial_residual>[\d.]+(?:e[+-]?\d+)?), Final residual = .*?, No Iterations (?P<uy_iterations>\d+)\n'
        r'smoothSolver:  Solving for Uz, Initial residual = (?P<uz_initial_residual>[\d.]+(?:e[+-]?\d+)?), Final residual = .*?, No Iterations (?P<uz_iterations>\d+)\n'
        r'GAMG:  Solving for p, Initial residual = (?P<p_iter1_1_initial_residual>[\d.]+(?:e[+-]?\d+)?), Final residual = .*?, No Iterations (?P<p_iter1_1_iterations>\d+)\n'
        r'GAMG:  Solving for p, Initial residual = (?P<p_iter1_2_initial_residual>[\d.]+(?:e[+-]?\d+)?), Final residual = .*?, No Iterations (?P<p_iter1_2_iterations>\d+)\n'
        r'time step continuity errors : sum local = [\d.]+(?:e[+-]?\d+)?, global = [+-]?[\d.]+(?:e[+-]?\d+)?, cumulative = [+-]?[\d.]+(?:e[+-]?\d+)?\n'
        r'GAMG:  Solving for p, Initial residual = (?P<p_iter2_1_initial_residual>[\d.]+(?:e[+-]?\d+)?), Final residual = .*?, No Iterations (?P<p_iter2_1_iterations>\d+)\n'
        r'GAMG:  Solving for p, Initial residual = (?P<p_iter2_2_initial_residual>[\d.]+(?:e[+-]?\d+)?), Final residual = .*?, No Iterations (?P<p_iter2_2_iterations>\d+)\n'
        r'time step continuity errors : sum local = [\d.]+(?:e[+-]?\d+)?, global = [+-]?[\d.]+(?:e[+-]?\d+)?, cumulative = [+-]?[\d.]+(?:e[+-]?\d+)?\n'
        r'smoothSolver:  Solving for omega, Initial residual = (?P<omega_initial_residual>[\d.]+(?:e[+-]?\d+)?), Final residual = .*?, No Iterations (?P<omega_iterations>\d+)\n'
        r'smoothSolver:  Solving for k, Initial residual = (?P<k_initial_residual>[\d.]+(?:e[+-]?\d+)?), Final residual = .*?, No Iterations (?P<k_iterations>\d+)\n'
...
def calculate_averages(matches):
    ...
    ux_iters = [int(m[2]) for m in matches]
    uy_iters = [int(m[4]) for m in matches]
    uz_iters = [int(m[6]) for m in matches]
    p_iter1_1_iters = [int(m[8]) for m in matches]
    p_iter1_2_iters = [int(m[10]) for m in matches]
    p_iter2_1_iters = [int(m[12]) for m in matches]
    p_iter2_2_iters = [int(m[14]) for m in matches]
    omega_iters = [int(m[16]) for m in matches]
    k_iters = [int(m[18]) for m in matches]
    execution_times = [float(m[19]) for m in matches]
...
with open('./pisoFoam.log', 'r') as file:

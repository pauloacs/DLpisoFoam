...
def assemble_prediction(
    ...
    for b in range(n_blocks):
        ...
        flow_bool = x_array[b, :, :, :, -1]  # SDF is always the last channel
        pred_field = array[b]
    ...
    outlet_has_fluid = np.any(x_array[:, :, :, -1, -1] != 0)  # SDF is always the last channel
    ...
    #    np.subtract(result, ddpPrev_grid, out=tmp0)
    ...
    if plot_results:
        _plot_sm_result(
            ...
            previous_change_in_delta_p=ddpPrev_grid,
            delta_u_change_weight=delta_U_change_grid if apply_deltaU_change_wgt else None,
            change_in_delta_p_before_weight=result_unweighted if apply_deltaU_change_wgt else None,
            weighted_change_in_delta_p=result if apply_deltaU_change_wgt else None,
...
def _plot_sm_result(
    ...
    - previous_change_in_delta_p: Previous change in delta p field (ddpPrev_grid)
    - delta_u_change_weight: Weight grid based on delta-U change
    - change_in_delta_p_before_weight: result - ddpPrev_grid

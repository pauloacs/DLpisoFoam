...
class Evaluation():
	...
	def computeOnlyOnce(self, sim):
		...
		self.grid_shape_x, self.grid_shape_y, self.grid_shape_z = utils_data.get_grid_shape(limits, self.delta)
	...
	def timeStep(
		...
		# --- Call _plot_sm_result for inspection ---
		try:
			# Use both sim and time for unique plot numbering
			call_count = None
			if 'sim' in locals() and 'time' in locals():
				call_count = sim * 10000 + time
			elif 'sim' in locals():
				call_count = sim
			elif 'time' in locals():
				call_count = time
			...
			# Pass the boolean mask (True for non-flow/obstacle, False for flow) to the plotting function
			obstacle_mask = (grid[0,:,:,:,3] == 0)
			assembly._plot_sm_result(
				assembled_pressure=deltap_res,
				previous_pressure_delta=None,
				delta_u_change_weight=None,
				pressure_delta_before_weight=None,
				weighted_pressure_delta_change=None,
			...
			print(f"[DEBUG] _plot_sm_result failed: {e}")

...
def _cluster_square_cylinders_3d(
    ...
    Args:
        ...
        grid_res: Grid resolution — scalar float or (dx, dy, dz) tuple.
    ...
    """
    from .data_processing import _unpack_grid_res
    ...
    # Use the largest per-axis resolution as the representative neighbourhood size.
    eps = max(dx, dy, dz) * eps_multiplier
...
def domain_dist(
    ...
    # Compute the domain limits from all boundaries
    all_x = np.concatenate([
        z_top[:, 0], z_bot[:, 0], y_top[:, 0], y_bot[:, 0]
    ])
    all_y = np.concatenate([
        z_top[:, 1], z_bot[:, 1], y_top[:, 1], y_bot[:, 1]
    ])
    all_z = np.concatenate([
        z_top[:, 2], z_bot[:, 2], y_top[:, 2], y_bot[:, 2]
    ])

    max_x = np.max(all_x) + dx
    min_x = np.min(all_x) - dx
    max_y = np.max(all_y) + dy
    min_y = np.min(all_y) - dy
    max_z = np.max(all_z) + dz
    min_z = np.min(all_z) - dz
    ...
    step = max(1, int(np.sqrt(obst.shape[0]) / 200)) if obst.shape[0] > 0 else 1

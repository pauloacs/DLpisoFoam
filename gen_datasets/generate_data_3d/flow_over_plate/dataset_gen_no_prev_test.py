...
def process_simulation(i, sim, path_to_sims, avance_list, hdf5_file, total_times, max_sim, max_patch):

    # # TEMPORARY workaround ...
    # if sim >= 7:
    #     path_to_sims = '/media/pasou/SSD/sim_cases'
    ...
    hdf5_file['sim_data'][i, ..., 2] = data['Uz']
    hdf5_file['sim_data'][i, ..., 3] = data['p']
    hdf5_file['sim_data'][i, ..., 4] = data['Cx']
    hdf5_file['sim_data'][i, ..., 5] = data['Cy']
    hdf5_file['sim_data'][i, ..., 6] = data['Cz']
    hdf5_file['sim_data'][i, ..., 7] = data['delta_Ux']
    hdf5_file['sim_data'][i, ..., 8] = data['delta_Uy']
    hdf5_file['sim_data'][i, ..., 9] = data['delta_Uz']
    hdf5_file['sim_data'][i, ..., 10] = data['delta_p']

    hdf5_file['y_bot_bound'][i, ..., 0] = data_y_bot['Cx']
    hdf5_file['y_bot_bound'][i, ..., 1] = data_y_bot['Cy']
    hdf5_file['y_bot_bound'][i, ..., 2] = data_y_bot['Cz']

    hdf5_file['z_bot_bound'][i, ..., 0] = data_z_bot['Cx']
    hdf5_file['z_bot_bound'][i, ..., 1] = data_z_bot['Cy']
    hdf5_file['z_bot_bound'][i, ..., 2] = data_z_bot['Cz']

    hdf5_file['y_top_bound'][i, ..., 0] = data_y_top['Cx']
    hdf5_file['y_top_bound'][i, ..., 1] = data_y_top['Cy']
    hdf5_file['y_top_bound'][i, ..., 2] = data_y_top['Cz']

    hdf5_file['z_top_bound'][i, ..., 0] = data_z_top['Cx']
    hdf5_file['z_top_bound'][i, ..., 1] = data_z_top['Cy']
    hdf5_file['z_top_bound'][i, ..., 2] = data_z_top['Cz']
    
    hdf5_file['obst_bound'][i, ..., 0] = data_obst['Cx']
    hdf5_file['obst_bound'][i, ..., 1] = data_obst['Cy']
    hdf5_file['obst_bound'][i, ..., 2] = data_obst['Cz']
...
def main():
    ...
    last_sim = 8
    total_times = 10
    hdf5_path = 'dataset_plate_piso_test_10t_9sim.hdf5'
    path_to_sims = 'simulation_data_test1'
    ...
    for i, sim in enumerate(tqdm(range(first_sim, last_sim + 1))):
        if i < 3:
           path_to_sims = '/home/pasou/Desktop/shared/backup_sims'
        else:
           path_to_sims = 'simulation_data_test1'
        process_simulation(i, sim, path_to_sims, avance_list, hdf5_file, total_times, max_n_cells_sim, max_n_cells_patch)

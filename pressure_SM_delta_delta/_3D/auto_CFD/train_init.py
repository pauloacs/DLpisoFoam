...
def main():
        ...
        # add_dddu_input: include delta_delta_U_diff (dddU) as model input
        try:
        ...
        # add_dp_prev_input: include delta_p_prev (previous pressure) as extra input
        try:
            from python_module import add_dp_prev_input
        except ImportError:
            add_dp_prev_input = False  # default: don't include previous pressure
        ...
        n_grid_channels = 2  # sdf, delta_p (+ optional flags)
        if add_U_input:
            n_grid_channels += 3  # U
        if add_dU_input:
        ...
        if add_ddu_input:
            ch_idx += 3
        dddu_idx = (ch_idx, ch_idx+3) if add_dddu_input else None
        if add_dddu_input:
            ch_idx += 3
        sdf_idx = ch_idx
        ch_idx += 1
        ...
        dp_prev_idx = ch_idx if add_dp_prev_input else None
        ...
        _ds_base = (dddu_idx[1] if add_dddu_input else (ddu_idx[1] if add_ddu_input else (dU_idx[1] if add_dU_input else (u_idx[1] if add_U_input else 0))))  # start of pressure channels in dataset
        ...
        dataset_ddp_prev_ch = dataset_dp_prev_ch + (1 if add_dp_prev_input else 0)  # ddp_prev position
        ...
        for step in range(n_samples):
            ...
            # Values inside non-flow regions are not important for training and can be set to zero or masked out. Here we set them to zero.
            mask = obst_bool[:,:,:, 0] == 0
        ...
        for var_idx in range(n_plot_vars):
            ...
            masked_arr = np.ma.array(grid_before[:, int(grid_before.shape[1] / 2), :, var_idx], mask= obst_bool[:, int(grid_before.shape[1] / 2), :, 0] == 0)
            plt.imshow(masked_arr, cmap='jet')
            ...
            plt.figure(figsize=(20, 6))
            masked_arr = np.ma.array(grid_before[int(grid_before.shape[0] / 2), :, :, var_idx], mask= obst_bool[int(grid_before.shape[0] / 2), ..., 0] == 0)
            plt.imshow(masked_arr, cmap='jet')
            plt.colorbar(label=var_names[var_idx])
        ...
        if single_block_mode:
            ...
            # Channel order: [U if add_U_input] [ddU if add_ddu_input] dddU sdf [delta_p_prev if add_dp_prev_input] delta_delta_p
            maxs_list = []
            ch = 0
            ...
            # Use ddp_idx explicitly (NOT -1 which becomes delta_p_prev when add_dp_prev_input=True)
